...
void CLinuxRendererGLES::RenderUpdateVideo(bool clear, DWORD flags, DWORD alpha)
{
  ...
  CLog::Log(LOGDEBUG,"CLinuxRendererGLES::RenderUpdateVideo");
...
void CLinuxRendererGLES::RenderSinglePass(int index, int field)
{
  ...
  YUVPLANES &planes = (m_format == RENDER_FMT_VDPAU_420 && m_currentField == FIELD_FULL) ? fields[field] : fields[FIELD_FULL];
  ...
  if (field != FIELD_FULL && m_format != RENDER_FMT_VDPAU_420)
...
//  m_sourceRect.y2 -= vdpau->crop.y2;
  ...
  // set textures
    ...
    glDisable(m_textureTarget);
    CLog::Log(LOGDEBUG, "UploadVDPAUTexture420 %d: img:%p\n", index, m_buffers[index].vdpau);
#endif
}
#endif
